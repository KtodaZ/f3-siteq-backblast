...
async function preprocessImageForFaceDetection(imageBuffer: Buffer) {
	try {
		...
		console.log(
			`Original image: ${metadata.width}x${metadata.height}, size: ${imageBuffer.length} bytes`,
		);
		...
		console.log(
			`Processed image: ${processedMetadata.width}x${processedMetadata.height}, size: ${processedBuffer.length} bytes`,
		);
 ...
 */
async function robustPhotoProcessing(
	photoId: number,
	s3Key: string,
	maxRetries = 3,
	...
	return await db.transaction(async (tx) => {
		try {
			...
			const preprocessResult =
				await preprocessImageForFaceDetection(originalImageBytes);
			const imageBytes = preprocessResult.buffer;

			console.log(
				`Using ${preprocessResult.enhanced ? "enhanced" : "original"} image for face detection`,
			);
			...
			const photoFaceRecords = faces.map((face, index) => {
				...
				return {
					...
					faceQuality: face.Quality?.Brightness
						? (face.Quality.Brightness + (face.Quality.Sharpness || 0)) / 2
						: face.Confidence,
			...
			await tx
				...
				.set({
					...
					enhancementApplied: preprocessResult.enhanced
						? "group_photo_optimization"
						: null,
				})
				.where(eq(photos.id, photoId));
...
export async function detectFacesInPhoto(
	...
	try {
		console.log(
			`Starting face detection for photo ${photoId} with key ${s3Key}`,
		);
