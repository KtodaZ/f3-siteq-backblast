...
export async function invokeFaceRecognition(
  ...
  try {
    ...
    const command = new InvokeCommand({
      FunctionName: env.FACE_RECOGNITION_LAMBDA_FUNCTION || "f3-face-recognition",
      Payload: JSON.stringify(payload),
    });
 ...
 */
export function isLikelyGroupPhoto(filename: string, fileSize: number): boolean {
  // Heuristics for group photo detection
  const groupKeywords = ['group', 'team', 'class', 'meeting', 'conference', 'party'];
