 ...
 */
async function robustFaceRecognition(photoId, s3Key, bucketName, collectionId, maxRetries = 3) {
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      console.log(`Face recognition attempt ${attempt} for photo ${photoId}`);
      
      const result = await processPhotoRecognition(photoId, s3Key, bucketName, collectionId);
      if (result.success) {
        return result;
      }
...
export const handler = async (event) => {
  try {
    ...
    if (!photoId || !s3Key || !bucketName || !collectionId) {
      throw new Error("Missing required parameters: photoId, s3Key, bucketName, collectionId");
    }

    // Use robust processing with retries
    const result = await robustFaceRecognition(photoId, s3Key, bucketName, collectionId);
    return result;
 ...
 */
async function processPhotoRecognition(photoId, s3Key, bucketName, collectionId) {
  ...
  return await db.transaction(async (tx) => {
    ...
    for (const match of faceMatches) {
      ...
      if (personRecord && personRecord.personId) {
        ...
        await tx.update(photoFaces)
          ...
          .where(
            and(
              eq(photoFaces.photoId, parseInt(photoId)),
    ...
    if (faceMatches.length === 0 && searchedFace) {
      console.log(`No matches found for face in photo ${photoId}, face can be indexed manually later`);
    }
...
export const indexNewFace = async (event) => {
  try {
    const { personId, s3Key, bucketName, collectionId, externalImageId } = event;
