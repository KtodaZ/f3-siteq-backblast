		...
		try {
			...
			try {
				await db
					...
					.where(eq(photos.id, Number.parseInt(photoId)));
			} catch (dbError) {
...
export const handler = async (event) => {
	try {
		...
		if (!photoId || !s3Key || !bucketName) {
			throw new Error(
				"Missing required parameters: photoId, s3Key, bucketName",
			);
		}
...
async function processPhoto(photoId, s3Key, bucketName) {
	...
	return await db.transaction(async (tx) => {
		...
		const photoFaceRecords = faces.map((face, index) => ({
			photoId: Number.parseInt(photoId),
