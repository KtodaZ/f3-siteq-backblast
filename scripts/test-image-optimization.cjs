...
async function testImageOptimization() {
	...
	try {
		...
		console.log(
			`  Optimized size: ${optimizedMeta.width}x${optimizedMeta.height}`,
		);
		console.log(`  File size: ${(optimized.length / 1024).toFixed(1)} KB`);
		console.log(
			`  Compression: ${(((imageBuffer.length - optimized.length) / imageBuffer.length) * 100).toFixed(1)}%\n`,
		);
		...
		for (const [size, buffer] of Object.entries(variants)) {
			...
			console.log(
				`  ${size}: ${meta.width}x${meta.height} (${(buffer.length / 1024).toFixed(1)} KB)`,
			);
		}
